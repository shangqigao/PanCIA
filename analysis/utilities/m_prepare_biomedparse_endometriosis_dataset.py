...
def convert_nifti_to_png(img_path, lab_path, output_img_dir, output_msk_dir, prefix_dict):
    ...
    if phase in ['axial', 'sagittal', 'coronal']:
        for i in range(img.shape[slice_axis]):
            ...
            if np.sum(slice_lab > 0) < 1: continue
            ...
            # save mask
            for val, target in labels.items():
                target = "+".join(target.split())
                if int(val) == 0: continue
                mask = slice_lab == int(val)
...
if __name__ == "__main__":
    ...
    root_dir = '/home/sg2162/rds/rds-ge-sow2-imaging-MRNJucHuBik/PanCancer/Endometriosis'
    dataset_names = [f for f in os.listdir(root_dir) if os.path.isdir(os.path.join(root_dir, f))]
    output_dir = '/home/sg2162/rds/rds-ge-sow2-imaging-MRNJucHuBik/PanCancer/BiomedParse_Endometriosis'
    print(f"Found {len(dataset_names)} datasets")
    for dataset in dataset_names:
        # if 'Endo' not in dataset: continue
        ...
        prefix_dict['labels'] = data_info['labels']

        # json_dir = f"{output_dir}/{dataset}"
