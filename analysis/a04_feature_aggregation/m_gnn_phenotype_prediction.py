...
class PhenotypeGraphDataset(Dataset):
    """loading graph data for pehnotype prediction
    """
...
class OmicsIntegrationArch(nn.Module):
    """define multi-omics integration architecture
    """
    def __init__(
            self, 
            dim_teachers, 
...
class PhenotypeGraphArch(nn.Module):
    """define Graph architecture for phenotype prediction
    Args:
    ...
    def train_batch(model, batch_data, on_gpu, loss, optimizer, kl=None):
        ...
        loss = loss(wsi_outputs, wsi_labels, VIparas, KAparas)
        loss.backward()
        optimizer.step()
...
class PhenotypeBayesGraphArch(nn.Module):
    """define Bayesian Graph architecture for phenotype prediction
    """
    def __init__(
            self, 
            dim_features, 
...
class R2TLoss(nn.Module):
    ...
    def forward(self, logits, labels, VIparas, KAparas):
        ...
        if KAparas is not None:
            ...
            loss_cls = loss_cls + self.tau_ka * loss_ka
        return loss_cls
