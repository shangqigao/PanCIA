        ...
        for key in self.data_types:
            ...
            for parent_idx, parent in enumerate(parents):
                for child_name, npz_path in parent.items():
                    ...
                    entry["parent_ids"].append(
                        torch.full((x.size(0),), parent_idx, dtype=torch.long)
                    )
...
def graph_sparse_diff(
    S: torch.Tensor,
    edge_index: torch.Tensor,
    edge_weight: torch.Tensor = None,
    diff_type: str = "laplacian"
) -> torch.Tensor:
    row, col = edge_index
    N = S.size(0)
    ...
    if diff_type == "laplacian":
        ...
        # Off-diagonal squared contribution:
        # w_ij^2 / (d_i d_j)
        contrib = edge_weight.pow(2) * deg_inv[row] * deg_inv[col]

        off_diag_sum = torch.zeros(N, device=row.device)
        off_diag_sum.scatter_add_(0, row, contrib)

        # Add diagonal term (1^2)
        row_norm_sq_node = 1.0 + off_diag_sum
        ...
        # Normalized Laplacian: L_norm S = S - D^-0.5 A D^-0.5 S
        diff = S - spmm(edge_index, norm_weight, N, N, S)
    
    elif diff_type == "gradient":
        if edge_weight is None:
            edge_weight = torch.ones(row.size(0), device=S.device)
        ...
        # Edge row norm squared (for each edge)
        row_norm_sq_edge = deg_inv_sqrt[row]**2 + deg_inv_sqrt[col]**2

        # Node-wise squared row norm: sum over incident edges
        row_norm_sq_node = torch.zeros(num_nodes, device=row.device)
        row_norm_sq_node.scatter_add_(0, row, row_norm_sq_edge)
        row_norm_sq_node.scatter_add_(0, col, row_norm_sq_edge)  # if undirected

        diff = (
            deg_inv_sqrt[row].unsqueeze(-1) * S[row]
            - deg_inv_sqrt[col].unsqueeze(-1) * S[col]
        )

    else:
        raise ValueError('diff_type must be "laplacian" or "gradient"')
...
def gamma_bin_pooling_weighted_normalized_soft_hierarchical(
    ...
    # --------------------------------------------------
    # 2️⃣ Compute Gamma quantile bin edges
    # --------------------------------------------------
    ...
    z = torch.erfinv(2 * probs - 1) * (2 ** 0.5)
    term = 1 - 1 / (9 * alpha_exp) + z / (3 * torch.sqrt(alpha_exp))
    bins_edges = (alpha_exp / beta_exp) * term.pow(3)
    bins_edges = bins_edges.clamp(min=1e-6)
    ...
    for bin_idx in range(num_bins):
        lower = bins_edges[bin_idx]
        upper = bins_edges[bin_idx + 1]
...
class MultiTaskGraphArch(nn.Module):
    ...
    def __init__(
        ...
        if aggregation == "MEAN":
            ...
            self.gate_nn_dict = nn.ModuleDict(
                {
                    k: Attn_Net_Gated(L=v, D=256, dropout=0.25, n_classes=2*v)
                    for k, v in self.dim_dict.items()
                }
            )
    ...
    def precision(self, loc, logvar):
        alpha = (self.alpha0 + 0.5) * torch.ones_like(loc)
...
class VILoss(nn.Module):
    ...
    def forward(self, loc, logvar, scale, pre, enc, dec):
