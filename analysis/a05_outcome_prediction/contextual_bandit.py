...
class ContextualBandit:
    ...
    def _policy_state_statistics(state):
        ...
        for column, name in enumerate(names):
            ...
            summaries.append(
                f"{name}: mean={values.mean():.3f}, SD={values.std():.3f}, "
                f"q05/50/95={q05:.3f}/{q50:.3f}/{q95:.3f}"
            )
    ...
    def _fit_policy_network(self, S, R, P, RP, E, T, train_idx, select_idx,
                            rp_cost, selection_data=None, verbose=True):
        """Fit on full-fit risks and select the policy on aligned OOF risks."""
        train_tensors = [
            torch.as_tensor(np.ascontiguousarray(values[train_idx]),
                            dtype=torch.float32, device=self.device)
            for values in (S, R, P, RP, E, T)
        ]
        selection_values = (S, R, P, RP, E, T)
        if selection_data is not None:
            selection_values = selection_data
        select_tensors = [
            torch.as_tensor(np.ascontiguousarray(values[select_idx]),
                            dtype=torch.float32, device=self.device)
            for values in selection_values
        ]
        ...
        for epoch in range(self.policy_epochs):
            ...
            # OOF validation loss is the only epoch-selection criterion.
            # OOF C-index remains a diagnostic so exploration stays part of
            ...
            if verbose and ((epoch + 1) % 10 == 0 or epoch == 0):
                print(
                    ...
                    f"OOF Val Loss = {val_loss:.4f}, "
                    f"OOF Val C-index = {val_cindex:.4f}, "
    ...
    def fit(self, X_rad, X_path, y):
        ...
        # the external test fold remains untouched.
        ...
        # One fixed split supports comparable policy early stopping and RP
        # evidence. Expert OOF risks also support the full-sample outer-EM
        # stopping proxy without using the external test fold.
        ...
        self.validation_soft_cindex_history = []
        best_em_checkpoint = None
        best_em_cindex = -np.inf
        patience_reference_cindex = -np.inf
        ...
        def fit_aligned_policy(verbose=True):
            ...
            # Full-fit risks train the policy and drive the weighted M-step.
            # Fold-aligned OOF risks provide an out-of-sample domain for
            # policy epoch selection and outer EM checkpointing.
            ...
            if verbose:
                ...
                print(
                    "Policy optimization source: full-fit training states/"
                    "risks; epoch selection source: fold-aligned OOF "
                    "states/risks"
                )
                print(
                    "Full-fit policy state statistics - "
                    + self._policy_state_statistics(S_policy)
                )
                print(
                    "OOF policy state statistics - "
                    + self._policy_state_statistics(S_oof)
                )
            best_val_loss = self._fit_policy_network(
                ...
                selection_data=(
                    S_oof, R_oof, P_oof, RP_oof, E_train, T_train
                ),
                verbose=verbose
            )
            ...
            soft_select_risk = (
                full_soft_probs[policy_select_idx, 0]
                * R_for_fit[policy_select_idx]
                + full_soft_probs[policy_select_idx, 1]
                * P_for_fit[policy_select_idx]
                + full_soft_probs[policy_select_idx, 2]
                * RP_for_fit[policy_select_idx]
            )
            selection_soft_cindex = concordance_index(
                T_train[policy_select_idx], -soft_select_risk,
                E_train[policy_select_idx].astype(bool)
            )
            expert_val_cindices = np.array([
                self._risk_cindex(
                    risk[policy_select_idx],
                    E_train[policy_select_idx], T_train[policy_select_idx]
                )
                for risk in (R_for_fit, P_for_fit, RP_for_fit)
            ], dtype=np.float64)

            # The full-OOF hard prediction selects the synchronized EM
            # checkpoint and controls outer early stopping. Selection-set OOF
            # scores remain useful diagnostics of held-out router behavior.
            ...
            hard_oof_risk = np.sum(hard_oof_probs * oof_risk_matrix, axis=1)
            soft_oof_risk = np.sum(soft_oof_probs * oof_risk_matrix, axis=1)
            full_oof_hard_cindex = self._risk_cindex(
                hard_oof_risk, E_train, T_train
            )
        ...
        for iteration in range(self.max_iterations):
            ...
            selection_soft_cindex = aligned['val_soft_cindex']

            self.validation_cindex_history.append(aligned['val_cindex'])
            self.validation_soft_cindex_history.append(
                selection_soft_cindex
            )
            ...
            print(
                "Personalized full-fit selection C-index - "
                f"Hard: {selection_cindex:.4f}, "
                f"Soft: {selection_soft_cindex:.4f}"
            )
            print(
                "Full-fit selection expert C-indices - "
            ...
            meaningful_improvement = (
                not np.isfinite(patience_reference_cindex)
            ...
            improvement_text = (
                "initial" if not np.isfinite(improvement)
            ...
            if (
                ...
                print(
                    "EM early stopping: full OOF hard C-index did not improve "
        ...
        if experts_updated_since_policy:
            ...
            print(
                ...
                f"{selection_oof_soft_cindex:.4f}; full OOF checkpoint "
        ...
        self.best_personalized_selection_cindex_ = best_em_checkpoint[
            'validation_cindex'
        ]
        ...
        self.best_full_oof_soft_cindex_ = best_em_checkpoint[
            'full_oof_soft_cindex'
        ]

        print(
            f"\nEM completed. Best checkpoint personalized selection "
            "C-index - "
        ...
        print(
            ...
            f"Hard: {self.best_full_oof_hard_cindex_:.4f}, "
            f"Soft: {self.best_full_oof_soft_cindex_:.4f}"
        )
